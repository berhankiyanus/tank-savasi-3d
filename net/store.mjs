...
export class AccountStore {
  ...
  async authenticate(token){await this.ready;if(typeof token!=='string'||!/^[a-f0-9]{64}$/.test(token))return null;const h=hash(token);if(this.pool){const r=await this.pool.query('SELECT * FROM arena_accounts WHERE token_hash=$1',[h]);return r.rows[0]||null;}return [...this.accounts.values()].find(a=>a.token_hash===h)||null;}
  ...
  async leaderboard(){if(this.pool)return (await this.pool.query('SELECT name,rating,games,wins FROM arena_accounts WHERE games>=5 ORDER BY rating DESC,id LIMIT 30')).rows;return [...this.accounts.values()].filter(a=>a.games>=5).sort((a,b)=>b.rating-a.rating).slice(0,30).map(({name,rating,games,wins})=>({name,rating,games,wins}));}
  async commit(m){
  ...
  async deleteAccount(id){
    if(!this.pool){this.accounts.delete(id);for(const result of this.results.values()){delete result.players[id];if(result.winner===id)result.winner=null;}return;}
